const testQuestions = require('./test_10_questions.cjs');

async function testServer() {
  const results = [];
  const startTime = Date.now();
  
  console.log('🚀 Iniciando teste com 10 perguntas diferentes...\n');
  
  for (let i = 0; i < testQuestions.length; i++) {
    const question = testQuestions[i];
    const questionStartTime = Date.now();
    
    console.log(`📝 Pergunta ${i + 1}/10: "${question}"`);
  ...
  console.log(`✅ Sucessos: ${successfulTests}/10`);
  console.log(`❌ Falhas: ${failedTests}/10`);
  console.log(`📈 Taxa de sucesso: ${(successfulTests/10*100).toFixed(1)}%`);
  ...
  const fs = require('fs');
  fs.writeFileSync('test_10_results.json', JSON.stringify({
    summary: {
      total: 10,
  ...
  console.log('\n💾 Resultados salvos em test_10_results.json');
}
